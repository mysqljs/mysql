...
var connection = common.createConnection({ typeCast: { date: false }});
...
var row;
connection.query({sql: 'SELECT * FROM type_casting', typeCast: { buffer: false }}, function(err, rows) {
  if (err) throw err;
...
process.on('exit', function() {
  tests.forEach(function(test) {
    ...
    if (expected instanceof Date) {
      assert.equal(typeof got, 'string', test.type);

      expected = String(expected);
      if (test.type === 'date') got += ' 00:00:00';
      got      = String(new Date(got));
