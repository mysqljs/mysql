...
Pool.prototype.getConnection = function (cb) {
  ...
  if (this.config.connectionLimit === 0 || this._allConnections.length < this.config.connectionLimit) {
    ...
    if (typeof connection.config.password === 'function') {
      const res = connection.config.password();
      if(typeof res === 'object' && res.constructor.name == 'Promise'){
        res.then((pass)=>{
          connection.config.password = pass;
          connectConnection(pool, connection, cb);
        })
...
Pool.prototype._needsChangeUser = function _needsChangeUser(connection) {
  ...
  return connConfig.user !== poolConfig.user
    ...
    || (typeof poolConfig.password === 'function'? false : connConfig.password !== poolConfig.password)
...
function connectConnection(pool, connection, cb) {
