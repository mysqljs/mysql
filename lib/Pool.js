...
Pool.prototype.getConnection = function (cb) {
  ...
  if (this.config.connectionLimit === 0 || this._allConnectionsLength < this.config.connectionLimit) {
    ...
    return connection.connect(function(err) {
      if (self._closed) {
        return self._raiseErrorToCB(cb, 'Pool is closed.');
      }
      if (err) {
        self._allConnectionsLength--;
        return self._raiseErrorToCB(cb, err);
      }
    ...
    (function () {
      ...
      connection.connect(function(err) {
        ...
        if (++attemptedConnect === initialSize) {
          ...
          self._allConnectionsLength = successfulConnect;
          self.emit('initialized', successfulConnect);
        }
      });
    })();
  }
