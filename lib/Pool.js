...
Pool.prototype.end = function (cb) {
  if (!this._initialized) {
    return this.once('initialized', this.end.bind(this, cb));
  }
...
Pool.prototype._initialize = function () {
  ...
  this.once('initialized', function () {
    var cb;
    while ((cb = this._connectionQueue.shift()) !== undefined) {
      process.nextTick(this.getConnection.bind(this, cb));
    }
  }.bind(this));
  ...
  if (initialSize <= 0) {
    ...
    return process.nextTick(this.emit.bind(this, 'initialized', 0));
  }
      ...
      connection.connect(function(err) {
        if (!err) {
          ...
          if (self._connectionQueue.length > 0) {
            var cb = self._connectionQueue.shift();
