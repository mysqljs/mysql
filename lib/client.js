...
Client.prototype.format = function(sql, params) {
  ...
  // Regex looks for the following (in top-down priority):
  ...
  var STATE = {
    ...
    SINGLE_QUOTE: 1,  // single quote literal
    DOUBLE_QUOTE: 2,  // double quote literal
    BACKTICK: 3       // backtick literal
  };

  // Since it's a very simple case of FSM with the following rules:
  //  * quote delimiters change state to their own only if current state is WHITESPACE
  //  * nothing but the same quote delimiter can change state back to WHITESPACE
  // we can simplify it a bit by using state switches 
  var stateSwitches = {
    '\'': STATE.SINGLE_QUOTE,
    '"': STATE.DOUBLE_QUOTE,
    '`': STATE.BACKTICK
  ...
  // set current state to default (whitespace)
  var state = STATE.WHITESPACE;

  // do the replacement
  sql = sql.replace(regex, function(str) {
    
    // if current match is a switch
    if (str in stateSwitches) {
      // if not in any literal
      if (state === STATE.WHITESPACE) {
        // switch state
        state = stateSwitches[str];

      // if the current state corresponds to current switch
      } else if (state === stateSwitches[str]) {
        // switch the state back (we're leaving the literal)
        state = STATE.WHITESPACE;
      }
    }

	  // if it's not placeholder
    if (str !== '?') {
      // put it back
       return str;
    }
