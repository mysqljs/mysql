...
Client.prototype.selectDB = function(db, cb) {
    ...
    this._enqueue(function() {
        if (self.debug) {
            console.log('Selecting the new database: ', db);
        }
...
Client.prototype.ping = function() {
    ...
    if (this._pinging) {
        if (this.debug) {
            console.log('Ping in progress, abort.');
        }
        return;
    }
    //Flag to check with setTimeout
    this._pinging = true;
    ...
    this._enqueue(function() {
        ...
        var packet = new OutgoingPacket(Client.COM_PING);
        //Issue the ping packet
        packet.writeNumber(1, Client.COM_PING);
        self.write(packet);
    }, function() {
        //This means that we got a response back..
        if (self.debug) {
            console.log('Ping Response, server is alive..');
        }
        self._pinging = false;
    });
    
    //Checking for a lack of a response
    setTimeout(function() {
        //We are still pinging
        if (self._pinging) {
            if (self.debug) {
                console.log('We are still pinging, reset connection.');
            }
            ...
            self.connect();
            //Set that we are not pinging
            self._pinging = false;
        }
    }, this.pingTimeout); //Configurable timeout
};
