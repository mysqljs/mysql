...
Client.prototype[Parser.RESULT_SET_HEADER_PACKET] = function(packet) {
  this._delegateToQuery(packet);
}

Client.prototype[Parser.ROW_DATA_PACKET] = function(row) {
  row.on('data', function (buffer, remaining) {
    console.log('row data (%d remaining): %s', remaining, buffer);
  });
}
