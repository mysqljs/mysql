...
Client.prototype._dequeue = function(err, result) {
  ...
  if (task.delegate instanceof Query) {
    if (err) {
      task.delegate.emit('error', err);
    } else {
      task.delegate.emit('end', result);
    }
  } else if (typeof task.delegate == 'function') {
    task.delegate(err, result);
  } else if (err) {
    this.emit('error', err);
  }
...
Client.prototype[Parser.GREETING_PACKET] = function(greeting) {
...
Client.prototype[Parser.ERROR_PACKET] = function(packet) {
...
Client.prototype[Parser.OK_PACKET] = function(packet) {
  var result = {
...
Client.prototype[Parser.RESULT_SET_HEADER_PACKET] =
Client.prototype[Parser.FIELD_PACKET] =
Client.prototype[Parser.EOF_PACKET] =
Client.prototype[Parser.ROW_DATA_PACKET] = function(packet) {
  var query = (this._queue.length)
    ? this._queue[0].delegate
    : null;

  if (!query) {
    this.emit('error', new Error('unexpected query packet, no packet was expected'));
    return;
  } else if (!(query instanceof Query)) {
    this.emit('error', new Error('unexpected query packet, another packet was expected'));
    return;
  }

  // query[packet.type](packet);
};
