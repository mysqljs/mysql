...
Client.prototype.insert = function(table, params, cb) {
	return this.query(this._buildQuery('INSERT', table, params), cb);
};

Client.prototype.update = function(table, params, where, cb) {
	var wsql = typeof where == 'object' ? this._parseSqlParts(where).join(' AND ') : where;
	var sql = this._buildQuery('UPDATE', table, params) + ' WHERE ' + wsql;
	return this.query(sql, cb);
};

Client.prototype.replace = function(table, params, cb) {
	return this.query(this._buildQuery('REPLACE', table, params), cb);
};
...
Client.prototype._parseSqlParts = function(params) {
	var parts = [];
	for (var i in params) {
		parts.push(i + ' = ' + this.escape(params[i]))
	}
	return parts;
};

Client.prototype._buildQuery = function(type, table, params) {
	var sql;
	if (type == 'INSERT' || type == 'REPLACE') sql = type + " INTO " + table + " SET ";
	else sql = "UPDATE " + table + " SET ";
	sql += this._parseSqlParts(params).join(', ');
	return sql;
};
