  ...
  for (; i < length; i++) {
    ...
    switch (state) {
      ...
      case Parser.COLUMN_VALUE_STRING:
        if (i + (packet.columnLength - packet.index) >= buffer.length) {
          packet.emit('data', buffer.slice(i, buffer.length), packet.columnLength - packet.index - buffer.length + i);
          i += buffer.length - 1;
          packet.index += buffer.length - 1;
        } else {
          packet.emit('data', buffer.slice(i, i + (packet.columnLength - packet.index)), 0);
          i += packet.columnLength - packet.index - 1;
          advance(Parser.COLUMN_VALUE_LENGTH);
        }
    ...
    if (state > Parser.PACKET_NUMBER && packet.received === packet.length) {
      if (packet.state != Parser.COLUMN_VALUE_STRING) {
        emitPacket();
      }
    }
  }
