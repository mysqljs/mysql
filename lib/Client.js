...
Client.prototype._connect = function() {
  ...
  this._socket
    ...
    .on('data', function(buffer) {
      try {
        var start = self._protocol.write(buffer)
      } catch (err) {
        self._onError(err);
        return;
      }

      self._parser.write(buffer.slice(start));
    })
...
Client.prototype.newQuery = function(sql, cb) {
  this._protocol.query({
    sql: sql,
  }, cb);
};
