...
var Handshake      = require('./protocol/Handshake');
...
Client.prototype._connect = function() {
  ...
  var self = this;
  this._handshake.on('packet', function(packet) {
    self.write(packet);
  });
  ...
  this._socket
    ...
    .on('data', this._parser.write.bind(this._parser))
...
Client.prototype._handlePacket = function(packet) {
  ...
  if (packet.type == Parser.GREETING_PACKET) {
    this._handshake.sendAuth(packet);
    return;
  }

  if (packet.type == Parser.USE_OLD_PASSWORD_PROTOCOL_PACKET) {
    this._handshake.sendOldAuth();
    return;
  }
