...
Query.prototype._isBlob = function(fieldType) {
  return fieldType === Query.FIELD_TYPE_TINY_BLOB ||
    fieldType === Query.FIELD_TYPE_MEDIUM_BLOB ||
    fieldType === Query.FIELD_TYPE_LONG_BLOB ||
    fieldType === Query.FIELD_TYPE_BLOB;
};
    ...
    case Parser.ROW_DATA_PACKET:
      ...
      packet.on('data', function(buffer, remaining) {
        if (!field) {
          ...
          field._isBlob = Query.prototype._isBlob(field.fieldType);
          if (field._isBlob) {
            row[field.name] = new Buffer(0);
          } else {
            row[field.name] = '';
          }
        }
        ...
        if (buffer) {
          if (field._isBlob) {
            var currentBuffer = row[field.name];
            var fieldBuffer = new Buffer(currentBuffer.length + buffer.length);
            currentBuffer.copy(fieldBuffer);
            buffer.copy(fieldBuffer);
            row[field.name] = fieldBuffer;
          } else {
