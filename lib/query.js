    ...
    case Parser.ROW_DATA_PACKET:
      ...
      packet.on('data', function(buffer, remaining) {
        if (!field) {
          ...
          // Until all data is received, the field value will contain a 2 elements array.
          // The first element is the (UTF8) string decoded so far
          // The second element is a string decoder which buffers binary data that cannot be decoded yet (it will be when next packet of data comes in)
          row[field.name] = ['', new string_decoder('utf8')];
        }

        if (buffer) {
          // Decode whatever UTF8 characters we can
          var decoder = row[field.name][1];
          row[field.name][0] += decoder.write(buffer);
        } else {
          row[field.name] = null;
        }
        ...
        // After all data has been decoded, get rid of the 2 elements array and only keep
        // the decoded UTF8 string
        if (row[field.name]) {
          row[field.name] = row[field.name][0];
        }
