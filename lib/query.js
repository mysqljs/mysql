      ...
      packet.on('data', function(buffer, remaining) {
        ...
        if (buffer) {
          if( row[field.name] == null ) {
            ...
            var newBuffer = new Buffer(buffer.length + row[field.name].length);
            row[field.name].copy(newBuffer, 0, 0);
            buffer.copy(newBuffer, row[field.name].length, 0);
            row[field.name] = newBuffer;
          }
        } else {
          row[field.name] = null;
        }
