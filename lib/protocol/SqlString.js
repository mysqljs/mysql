...
SqlString.escape = function(val, stringifyObjects) {
  ...
  val = SqlString.escapeString(val);
  return "'"+val+"'";
};

SqlString.escapeField = function(val) {
  if (Array.isArray(val)) {
    return SqlString.arrayToList(val, true);
  }
...
SqlString.arrayToList = function(array, isField) {
	return array.map(function(v) {
		if (Array.isArray(v)) return '(' + SqlString.arrayToList(v, isField) + ')';
		return isField ? SqlString.escapeField(v, true) : SqlString.escape(v, true);
	}).join(', ');
};
...
SqlString.objectToValues = function(object) {
  ...
  for (var key in object) {
    ...
    values.push(SqlString.escapeField(key) + ' = ' + SqlString.escape(value, true));
  }
