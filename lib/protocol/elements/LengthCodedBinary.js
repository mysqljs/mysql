...
Util.inherits(LengthCodedBinary, Element);
...
LengthCodedBinary.prototype.copy = function(buffer, offset) {
  ...
  if (length === 9) {
    ...
    // Set last byte to 0 as we don't support values that large
    buffer[offset + 8] = 0;
    return;
  }
};

LengthCodedBinary.prototype.write = function(buffer, start, end) {
  var offset = 0;

  while (offset + start < end) {
    var byte = buffer[offset + start];
    ...
    if (this.bytesWritten === 0) {
      ...
      if (byte === 252) {
        this.length = 3; // 16 Bit Value
      } else if (byte === 253) {
        this.length = 4; // 24 Bit Value
      } else if (byte === 254) {
        this.length = 9; // 24 Bit Value
      }
    } else {
      this.value += Math.pow(256, this.bytesWritten - 1) * byte;

      if (this.value >= IEEE_754_BINARY_64_PRECISION) {
        throw new Error(
          'LengthCodedBinary.SizeExceeded: JS precision range exceeded,' +
          'number is > 53 bit. Byte ' + this.bytesWritten + ' = ' + byte + '.'
        );
      }
    }

    this.bytesWritten++;
    if (this.bytesWritten === this.length) return true;
