...
Protocol.prototype._enqueue = function(sequence) {
  ...
  if (this._queue.length === 1) {
    ...
    // socket is actually useful, we need to wait for a response
    this._connection._socket.ref();
...
Protocol.prototype._dequeue = function(sequence) {
  ...
  var sequence = this._queue[0];
  if (!sequence) {
    // nothing interesting left on the socket, the program can safely close
