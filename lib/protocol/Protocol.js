...
Protocol.prototype._parsePacket = function() {
  ...
  if (!sequence) {
    ...
    if (this._config.debug) {
      console.log('=== Received packet without sequence in queue ===');
      console.log(parser._packetHeader);
      console.log(parser._buffer.slice(parser._packetOffset, parser._packetEnd));
      console.log('');
    }
  ...
  if (this._config.debug) {
    packet._header = parser._packetHeader;
    packet._raw    = parser._buffer.slice(parser._packetOffset, parser._packetEnd);
  }

  // Special case: Faster dispatch, and parsing done inside sequence
  if (Packet === Packets.RowDataPacket) {
    sequence.RowDataPacket(packet, parser, this._connection);
