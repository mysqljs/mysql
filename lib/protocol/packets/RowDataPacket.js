...
RowDataPacket.prototype.parse = function(parser, fieldPackets, typeCast, nestTables) {
  for (var i = 0; i < fieldPackets.length; i++) {
    ...
    var value = (typeCast)
      ? this._typeCast(fieldPacket, parser, typeCast)
...
RowDataPacket.prototype._typeCast = function(field, parser, types) {
  switch (field.type) {
    ...
    case Types.NEWDATE:
      ...
      if (!types.date) return dateString;
      if (dateString === null) {
        return null;
      }
    ...
    case Types.DOUBLE:
      ...
      if (!types.number) return numberString;
      return (numberString === null)
        ? numberString
        : Number(numberString);
    case Types.BIT:
      if (!types.buffer) return parser.parseLengthCodedString();
    ...
    case Types.BLOB:
      return (field.charsetNr === Charsets.BINARY && types.buffer)
