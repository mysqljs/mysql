...
Query.prototype._finish = function(err) {
  ...
  for(var i = 0, len = this._results.length, result = this._results[i]; i < len; i++, result = this._results[i]) {
    if(result instanceof Packets.OkPacket) {
      if(!i || this._results[i - 1] instanceof Packets.OkPacket) {
        this._results.push(undefined);
        for(var y = len, tmp; y > i; y--) {
          tmp = this._results[y];
          this._results[y] = this._results[y - 1];
          this._results[y - 1] = tmp;
        }
        len++;
        this._results[i] = new ResultSet();
        i--;
      }
    }
    else {
      result._parent = this._results;
      args.push(result);
    }
  }
