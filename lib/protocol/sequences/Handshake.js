...
Handshake.prototype['AuthSwitchRequestPacket'] = function (packet) {
  if (packet.authMethodName === 'caching_sha2_password') {
    var challenge = packet.authMethodData.slice(0, 20);

    this.emit('packet', new Packets.AuthSwitchResponsePacket({
      data: Auth.sha2Token(this._config.password, challenge)
    }));
  } else if (packet.authMethodName === 'mysql_native_password') {
    var challenge = packet.authMethodData.slice(0, 20);
    ...
    this.emit('packet', new Packets.AuthSwitchResponsePacket({
      data: Auth.scramble323(challenge, this._config.password)
    }));
  } else {
    var err = new Error(
...
Handshake.prototype['PerformFullAuthenticationPacket'] = function() {
  ...
  if (!this._config.ssl && !this._config.socketPath) {
    ...
    if (secureAuth && typeof secureAuth.key === 'string') {
      // Use the provided authentication RSA public key with any given padding type.
...
Handshake.prototype['AuthMoreDataPacket'] = function(packet) {
  var secureAuth = { key: packet.data, padding: this._config.secureAuth.padding || Constants.RSA_PKCS1_OAEP_PADDING };
