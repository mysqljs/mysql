...
Connection.prototype.query = function(sql, values, cb) {
  ...
  var self = this;
  options.typeCast = Object.keys(this.config.typeCast).reduce(function(o, type) {
    if (typeof options.typeCast === 'boolean') o[type] = options.typeCast;
    else if (type in options.typeCast) o[type] = options.typeCast[type];
    else o[type] = self.config.typeCast[type];
    return o;
  }, {});
