...
Connection.prototype.beginTransaction = function(cb) {
  ...
  var query = Connection.createQuery('BEGIN', null, cb);
...
Connection.prototype.commit = function(cb) {
  if (!this.inTransaction) {
    // No transaction to commit, call back
    return cb();
  }

  this._implyConnect();

  var query = Connection.createQuery('COMMIT', null, cb);
  query._connection = this;
...
Connection.prototype.rollback = function(cb) {
  if (!this.inTransaction) {
    // No transaction to roll back, call back
    return cb();
  }

  this._implyConnect();

  var query = Connection.createQuery('ROLLBACK', null, cb);
  query._connection = this;
