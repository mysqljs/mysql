...
if (false&&Net.Socket.prototype.unpipe) {
  // node.js 0.10-like
  Connection.prototype._startTLS = function _startTLS(onSecure) {
    ...
    var eventNames = ['data', 'drain', 'close', 'end', 'readable'];
    var securePair = tls.createSecurePair(credentials, false);

    // unpipe
    this._protocol.unpipe(this._socket);
    this._socket.unpipe(this._protocol);
    ...
    // cleartext <-> protocol
    this._protocol.pipe(securePair.cleartext);
    securePair.cleartext.pipe(this._protocol);

    // add error handlers
    securePair.encrypted.on('error', this._handleNetworkError.bind(this));
    securePair.cleartext.on('error', this._handleNetworkError.bind(this));
  ...
  Connection.prototype._startTLS = function _startTLS(onSecure) {
    ...
    // unpipe; thanks node.js 0.8
    for (var i = 0; i < eventNames.length; i++) {
      this._protocol.removeAllListeners(eventNames[i]);
      this._socket.removeAllListeners(eventNames[i]);
    }
