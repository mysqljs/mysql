...
ConnectionConfig.parseCastMap = function(castMap) {
  if (castMap === undefined) { 
    return Types.DEFAULT_CAST_MAP;
  } else if (typeof castMap === 'object') {
    var _castMap = Types.DEFAULT_CAST_MAP; 
    // overwrite default cast map with the supplied cast map
    Object.keys(castMap).forEach(function(mysqlType) {
      _castMap[mysqlType] = castMap[mysqlType];
    });
    return _castMap;
  } else { 
    return {}; 
