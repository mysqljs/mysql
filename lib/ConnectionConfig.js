...
function ConnectionConfig(options) {
  ...
  var types = ['date', 'number', 'buffer'];
  this.typeCast = 'typeCast' in options ? options.typeCast : true;
  if (typeof this.typeCast === 'boolean') {
    var enableCast = this.typeCast;
    this.typeCast = {};
    types.forEach(function(type) {
      this.typeCast[type] = enableCast;
    }, this);
  }
  else if (typeof this.typeCast === 'object') {
    types.forEach(function(type) {
      if (typeof this.typeCast[type] === 'undefined')
        this.typeCast[type] = true;
    }, this);
  }
