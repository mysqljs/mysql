...
* `dateStrings`: Force date types (TIMESTAMP, DATETIME, DATE) to be returned as strings rather then
   inflated into JavaScript Date objects. Specifying `"DATE"` will limit this behavior to DATE fields 
   while returning Javascript Date objects for TIMESTAMP and DATETIME fields. (Default: `false`)
